...
describe("sandbox extension", () => {
  ...
  function createContextWithSpies(overrides?: Parameters<typeof createMockContext>[0]) {
    const ctx = createMockContext(overrides);
    return {
      ctx,
      confirm: vi.spyOn(ctx.ui, "confirm"),
      input: vi.spyOn(ctx.ui, "input"),
    };
  }
  ...
  describe("git push", () => {
    it("prompts for git push", async () => {
      const { ctx, confirm } = createContextWithSpies();
    ...
    it("requires typed confirm for git force push", async () => {
      const { ctx, input } = createContextWithSpies();
      input.mockResolvedValue("DANGEROUSLY_ALLOW");
  ...
  describe("dangerouslyAllowCommands flag", () => {
    it("bypasses all guards when flag is set", async () => {
      ...
      const { ctx, confirm, input } = createContextWithSpies();
