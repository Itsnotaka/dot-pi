...
function hasShortFlag(args: string[], flag: string): boolean {
	return args.some((arg) => arg.startsWith("-") && !arg.startsWith("--") && arg.includes(flag));
}
...
function hasRecursiveFlag(flags: string[]): boolean {
	const shortFlags = flags.filter((arg) => arg.startsWith("-") && !arg.startsWith("--"));
	const hasShort = shortFlags.some((arg) => arg.includes("r") || arg.includes("R"));
...
function matchesSqlDestructive(segment: SegmentInfo): boolean {
	const sqlPattern = /\b(drop\s+(table|database|schema)|truncate|delete\s+from)\b/i;
...
const commandRules: CommandRule[] = [
	{
		...
		match: (command) => {
			...
			const targets = [...before.filter((arg) => !arg.startsWith("-")), ...after];
			const hasRecursive = hasRecursiveFlag(flags);
			const nuclear = targets.some((target) => isNuclearTarget(target)) || hasLongFlag(flags, "no-preserve-root");
	...
	},
	{
		...
		severity: "deny",
		match: (command) => Boolean(command.commandName && command.commandName.startsWith("mkfs")),
	},
	{
		label: "Disk write (dd)",
		severity: "deny",
		match: (command) =>
			...
			command.args.some((arg) => arg.startsWith("of=/dev/") || arg.startsWith("if=/dev/") || arg.includes("/dev/")),
	},
	{
		label: "Disk write (dd)",
		severity: "ask",
	...
	},
	{
		...
		match: (command) => {
			...
			const isForce = hasLongFlag(rest, "force") || hasLongFlag(rest, "force-with-lease") || hasShortFlag(rest, "f");
			return !isForce;
		},
	},
	{
	...
	},
	{
		...
		match: (command) => {
			if (command.commandName !== "git") return false;
			const { subcommand, rest } = getGitSubcommand(command.args);
			return subcommand === "clean" && (hasShortFlag(rest, "f") || hasLongFlag(rest, "force"));
		},
	},
	{
		...
		match: (command) => {
			...
			return rest.slice(markerIndex + 1).some((arg) => arg === "." || arg === "./");
		},
	},
	{
		...
		match: (command) => {
			...
			if (!["npm", "pnpm", "yarn", "bun"].includes(command.commandName)) return false;
	...
	},
	{
		label: "PyPI publish",
		severity: "ask",
		match: (command) => command.commandName === "twine" && command.args.includes("upload"),
	},
	{
		label: "RubyGems publish",
		severity: "ask",
		match: (command) => command.commandName === "gem" && command.args.includes("push"),
	},
	{
		label: "Cargo publish",
		severity: "ask",
		match: (command) => command.commandName === "cargo" && command.args.includes("publish"),
	},
	{
		label: "Deno publish",
		severity: "ask",
		match: (command) => command.commandName === "deno" && command.args.includes("publish"),
	},
	{
		label: "JSR publish",
		severity: "ask",
		match: (command) => command.commandName === "jsr" && command.args.includes("publish"),
	},
	{
		label: "Hex publish",
		severity: "ask",
		match: (command) => command.commandName === "mix" && command.args.includes("hex.publish"),
	},
	{
		label: "NuGet publish",
		severity: "ask",
	...
	},
	{
		...
		match: (command) => {
			...
			if (["netlify", "firebase", "fly", "flyctl", "railway"].includes(command.commandName)) {
	...
	},
	{
		...
		match: (command) =>
			...
			(getFirstNonFlagArg(command.args) === "apply" || getFirstNonFlagArg(command.args) === "destroy"),
	},
	{
		...
		match: (command) =>
			...
			(getFirstNonFlagArg(command.args) === "up" || getFirstNonFlagArg(command.args) === "destroy"),
	},
	{
	...
	},
	{
		...
		severity: "ask",
		match: (command) =>
			command.commandName === "aws" && command.args.some((arg) => /\b(delete|terminate|destroy|rm|purge)\b/.test(arg)),
	},
	{
	...
	},
	{
		...
		severity: "ask",
		match: (command) => command.commandName === "chmod" && command.args.includes("777"),
	},
	{
		label: "Recursive chmod",
		severity: "ask",
		match: (command) =>
			command.commandName === "chmod" && (hasShortFlag(command.args, "R") || hasLongFlag(command.args, "recursive")),
	},
	{
	...
	},
	{
		...
		match: (command, segment) =>
			command.commandName !== null && SQL_CLIENTS.has(command.commandName) && matchesSqlDestructive(segment),
	},
	{
		...
		match: (command) => {
			if (command.commandName === "docker") {
				...
				const detached = command.args.includes("-d") || command.args.includes("--detach");
...
function evaluateCommand(command: string): RuleMatch[] {
	...
	const segments = splitTopLevel(command).map((segment) => parseSegment(segment));

	for (const segment of segments) {
		for (const rule of segmentRules) {
			if (rule.match(segment)) matches.push({ label: rule.label, severity: rule.severity });
		}
		for (const part of segment.pipeline) {
			for (const rule of commandRules) {
				if (rule.match(part, segment)) matches.push({ label: rule.label, severity: rule.severity });
			}
		}
	}
...
export default function (pi: ExtensionAPI) {
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		const severity: Severity = matches.some((m) => m.severity === "deny") ? "deny" : "ask";
		...
		const choice = await ctx.ui.select(`⚠️ ${tag}\n${cmd}`, ["Allow once", "Always allow (session)", "Reject"]);

		if (!choice || choice === "Reject") {
			return { block: true, reason: `User rejected command (${tag})` };
		}

		if (choice === "Always allow (session)" && !allowedCommands.has(normalized)) {
